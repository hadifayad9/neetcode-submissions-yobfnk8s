...
class Twitter {
  ...
  getNewsFeed(userId) {
    ...
    this.followeeList[userId].forEach((element) => {
      if (
        this.twitterList.hasOwnProperty(element) &&
        this.twitterList[element].length > 0
      ) {
        maxHeap.push([
          this.twitterList[element][this.twitterList[element].length - 1][1],
          element,
          this.twitterList[element].length - 1,
        ]);
      }
    });
