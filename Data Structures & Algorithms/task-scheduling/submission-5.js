...
class Solution {
  ...
  leastInterval(tasks, n) {
    ...
    for (let i = 0; i < tasks.length; i++) {
      maxFreq[tasks[i]] = (maxFreq[tasks[i]] || 0) + 1;
    }
    for (const key in maxFreq) {
      maxHeap.push(maxFreq[key]);
    }
    let time = 0;
    let dq = [];
    while (maxHeap.size() !== 0 || dq.length !== 0) {
      time++;
      if (maxHeap.size() > 0) {
        let task = maxHeap.pop();
        task--;
        if (task !== 0) {
          dq.push([task, n + time]);
        }
      }
      if (dq.length > 0 && time === dq[0][1]) {
        maxHeap.push(dq.shift()[0]);
      }
    }
    return time;
  }
