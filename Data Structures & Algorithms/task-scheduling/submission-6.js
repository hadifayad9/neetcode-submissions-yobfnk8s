...
class Solution {
  ...
  leastInterval(tasks, n) {
    let obj = {};
    let tasksSched = new MaxHeap();
    for (let i = 0; i < tasks.length; i++) {
      obj[tasks[i]] = (obj[tasks[i]] || 0) + 1;
    }
    for (const [key, value] of Object.entries(obj)) {
      tasksSched.push(value);
    }
    let total = 0;
    let queue = [];
    while (tasksSched.size() > 0 || queue.length > 0) {
      total++;
      if (tasksSched.size() > 0) {
        let cpuCycle = tasksSched.pop();
        cpuCycle = cpuCycle - 1;
        if (cpuCycle !== 0) {
          queue.push([cpuCycle, n + total]);
        }
      }
      if (queue.length > 0 && queue[0][1] === total) {
        let task = queue.shift();
        tasksSched.push(task[0]);
      }
    }
    return total;
  }
